...
- (void)handleGetScreenSize:(nonnull GetScreenSizeIntent *)intent completion:(nonnull void (^)(GetScreenSizeIntentResponse * _Nonnull))completion {
    ...
    if ([self.springBoardSocket isConnected])
    {
        ...
        response.result = [[IntentSize alloc] initWithIdentifier:@"com.tlinkauto.tlinkauto.shortcutext.types.task-result" displayString:NSLocalizedFormatString(@"IntentSizeResultDisplayString", success, width, height)];
