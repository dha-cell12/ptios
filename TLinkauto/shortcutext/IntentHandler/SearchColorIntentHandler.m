...
- (void)handleSearchColor:(nonnull SearchColorIntent *)intent completion:(nonnull void (^)(SearchColorIntentResponse * _Nonnull))completion {
    ...
    if ([self.springBoardSocket isConnected])
    {
        ...
        response.result = [[RGBXY alloc] initWithIdentifier:@"com.tlinkauto.tlinkauto.shortcutext.types.task-result" displayString:NSLocalizedFormatString(@"RGBResultDisplayString", success, x, y, red, green, blue)];
