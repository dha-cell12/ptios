...
- (void)handlePickColor:(nonnull PickColorIntent *)intent completion:(nonnull void (^)(PickColorIntentResponse * _Nonnull))completion {
    ...
    if ([self.springBoardSocket isConnected])
    {
        ...
        response.result = [[RGB alloc] initWithIdentifier:@"com.tlinkauto.tlinkauto.shortcutext.types.task-result" displayString:NSLocalizedFormatString(@"RGBResultDisplayString", success, red, green, blue)];
