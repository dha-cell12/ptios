...
- TrollStore foreground discovery uses resolver v10. A task-11 launch intent is checked first; otherwise a 96-cell screen fingerprint detects material presentation changes, recent confirmed app contexts are probed before the full running-app set, and a newly selected app must pass a second AX observation. Candidates must also be visible LaunchServices applications: hidden/background system UI services such as `assistivetouchd` are excluded even when they expose a persistent AX tree. The resolver stops after the first filtered non-stale candidate passes confirmation, caches LaunchServices for 60 seconds and the PID inventory for 750 ms, and force-refreshes that inventory during an unsettled app transition. If the fingerprint reports a transition but the previous app remains the sole eligible AX candidate, v10 performs a bounded retry and accepts it only after a third stable observation (`ax_snapshot_previous_stable_v4`); this avoids a false `ui_frontmost_transition_unsettled` caused by animations or dynamic content without blindly trusting stale state. SBS/FBS remain available to legacy tasks 34/51 but are excluded from the UI-tree hot path after bounded AX retries. A genuinely stale or ambiguous app is still reported as `ui_frontmost_transition_unsettled` or `ui_frontmost_ax_probe_ambiguous`.
