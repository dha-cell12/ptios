...
- Widget-assisted boot wake is implemented for the TrollStore build. `TLinkBootWidget.appex` requests a repeating WidgetKit timeline, checks the protected boot-enable marker and task port `6000`, then uses the private SpringBoardServices launch API with the unlock option to open StreamControl when the service is absent. Settings -> Boot Script stores the selected JavaScript as the priority `00-boot-script` autolaunch entry. This path requires the user to add the **TLinkauto Boot Wake** widget to the Home Screen and remains best effort because WidgetKit controls actual refresh/launch timing; it is not a platformized LaunchDaemon. BGTaskScheduler remains the fallback after first launch.
