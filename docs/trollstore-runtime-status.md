...
- Keyboard/text: task `24` uses `clipboardd` v10 on port `6012` with private background Pasteboard entitlements. Every write is read-back verified before `streamd` trusts daemon reads. Subtask `1` inserts text using background clipboard plus HID `Command+V`; subtask `5` pastes the existing clipboard; subtasks `3/4` use HID arrows and Backspace. Devices that ignore the Pasteboard entitlements fall back to the foreground app bridge on port `6013`. Show/hide keyboard remains `limited_on_trollstore` because it needs the rootfull SpringBoard keyboard observer.
- Background UI bridge: `clipboardd` v10 receives toast/alert/dialog fallback events from `streamd`. Background toast first uses a noninteractive UIDaemon `UIWindow`, preserving the rootfull position argument (`0` top, `1` center, `2` bottom). If that window cannot be created, toast falls back to `CFUserNotificationDisplayAlert`. Alert/dialog continue to use the system alert fallback because arbitrary app-process overlays are unavailable while StreamControl is in the background. Foreground still uses the normal UIKit overlay.
...
```powershell
Invoke-TLinkTask -HostIP $iphoneIP -Task "220;;Toast TOP v10;;6;;0;;16"
Start-Sleep -Milliseconds 700
Invoke-TLinkTask -HostIP $iphoneIP -Task "249"
Invoke-TLinkTask -HostIP $iphoneIP -Task "220;;Toast CENTER v10;;6;;1;;16"
Invoke-TLinkTask -HostIP $iphoneIP -Task "220;;Toast BOTTOM v10;;6;;2;;16"
Invoke-TLinkTask -HostIP $iphoneIP -Task "12TLinkauto;;Background alert test;;3"
Invoke-TLinkTask -HostIP $iphoneIP -Task "401"
Invoke-TLinkTask -HostIP $iphoneIP -Task "249"
```

The three toasts should appear at the top, center, and bottom without bringing
StreamControl to the foreground. While a six-second toast is visible, task `249`
should report `version=10`, `toast_overlay_visible=1`, the requested
`toast_position`, and `background_visual_last=uidaemon_toast_presented_position_N`.
Alert remains a SpringBoard system alert. If diagnostics say the toast window is
visible but nothing is drawn above the foreground app, that iOS build is not
compositor-hosting the UIDaemon window and the private window-hosting path needs
further device-specific work.
