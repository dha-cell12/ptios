...
- Keyboard/text: task `24` supports clipboard text get/set and clipboard image through a foreground/alive `StreamControl.app` app-side bridge. Insert, paste, delete, cursor movement, and show/hide keyboard depend on the rootfull SpringBoard keyboard observer and return `limited_on_trollstore` on TrollStore instead of silent success.
