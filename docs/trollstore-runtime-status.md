...
- Vision OCR CPU-only remains experimental for the former `420f`/worker crash issue documented in `plan.md`; task `91` Tesseract is still the stable/default OCR path. P1 keeps the CPU-only `app_cpu`/`worker_cpu` profiles, and P2 established the compact-BGRA foreground app canary. P3 now routes `app_cpu` and `xxt_compat` to the headless OCR endpoint in `TLinkUIService.app` on port `6018`, so StreamControl may remain backgrounded while the target app stays foreground. The OCR endpoint creates no scene or window; the black-screen FrontBoard path remains disabled and toast stays on port `6017`. Task `275` performs a three-second live UI-service bridge probe. `Collect-TLinkVisionOCRQualification.ps1` provides a bounded 20 Fast + 1 Accurate + 1 large Fast background gate with task `97` postflight after every request; promotion remains blocked until that artifact and manual text/coordinate checks pass. Task `273/274` reads/clears the bounded debug log. See `docs/ocr-p1-cpu-only.md`, `docs/ocr-p2-xxt-compat.md`, and `docs/ocr-p3-background-uiservice.md`.
