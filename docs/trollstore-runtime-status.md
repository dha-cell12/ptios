...
- Keyboard/text: task `24` supports clipboard text get/set and clipboard image through a foreground broker. The persistent `clipboardd` on port `6012` is diagnostic-only because iOS rejects its background writes and its reads can be empty; normal clipboard data never uses it. `streamd` briefly opens `StreamControl.app`, sends the operation to the app bridge on port `6013`, then restores a foreground app cached within the last 60 seconds when possible. Insert, paste, delete, cursor movement, and show/hide keyboard still depend on the rootfull SpringBoard keyboard observer and return `limited_on_trollstore` instead of silent success.
