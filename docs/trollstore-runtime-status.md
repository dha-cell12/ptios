...
- Service bootstrap: opening StreamControl always ensures `streamd` is running. Task `97` exposes `serviceVersion=14`; the app replaces a responding process when its version is stale or its executable belongs to an older app bundle, then starts the bundled binary through `privhelper`.
