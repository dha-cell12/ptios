...
- Vision OCR remains experimental for the former `420f`/worker crash issue documented in `plan.md`; task `91` Tesseract is still the stable/default OCR path. P1 keeps the CPU-only `app_cpu`/`worker_cpu` profiles. P2 adds opt-in `xxt_compat`, which uses a plain Vision request, direct `CGImage`, and automatic compute selection inside the isolated worker. Task `273/274` reads/clears its bounded debug log. See `docs/ocr-p1-cpu-only.md` and `docs/ocr-p2-xxt-compat.md`.
