...
- Keyboard/text: task `24` uses `clipboardd` v5 on port `6012` with private background Pasteboard entitlements. Every write is read-back verified before `streamd` trusts daemon reads. Subtask `1` inserts text using background clipboard plus HID `Command+V`; subtask `5` pastes the existing clipboard; subtasks `3/4` use HID arrows and Backspace. Devices that ignore the Pasteboard entitlements fall back to the foreground app bridge on port `6013`. Show/hide keyboard remains `limited_on_trollstore` because it needs the rootfull SpringBoard keyboard observer.
- Background UI bridge: `clipboardd` v5 also receives toast/alert/dialog fallback events from `streamd`. While StreamControl is active it keeps the UIKit overlay; after the foreground heartbeat expires, the daemon posts a local notification under the app identity. Notification permission is requested once when StreamControl is opened after install.
...
StreamControl to the foreground. Task `249` should report `version=5`,
`background_ui_bridge=1`, and the notification authorization state.
