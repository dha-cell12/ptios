...
- Keyboard/text: task `24` supports clipboard text get/set and clipboard image through persistent UIKit companion `clipboardd` on loopback port `6012`. Open `StreamControl.app` once after install to start the services; the app does not need to remain foreground. Insert, paste, delete, cursor movement, and show/hide keyboard still depend on the rootfull SpringBoard keyboard observer and return `limited_on_trollstore` instead of silent success.
