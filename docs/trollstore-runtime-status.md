...
- UI-service hosted lifecycle (v7): the service initializes GraphicsServices and BackBoardServices, instantiates `TLinkUIServiceApplication` as the principal `UIApplication`, then enters `__completeAndRunAsPlugin`. This matches the hidden system-app hosting model used by XXTUIService and avoids the failed `UIApplicationMain`/foreground-restore path. The persistent plugin owns a hidden-tagged, non-key, pass-through window; an explicit **Restart streamd** brings port `6000` up first, then performs UI/clipboard/VPN recovery in a detached helper so auxiliary startup cannot block streamd.
...
live `uiservice_ready;;version=7` response containing `uid=501`, `euid=501`,
`launch_mode=UIKitPluginHosted`, `plugin_complete=1`, `window_ready=1`, and
