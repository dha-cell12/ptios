...
- Vision OCR CPU-only remains experimental for the former `420f`/worker crash issue documented in `plan.md`; task `91` Tesseract is still the stable/default OCR path. P1 keeps the CPU-only `app_cpu`/`worker_cpu` profiles. After the 2026-08-29 crash report identified `CI::GLContext` initialization in the headless root worker, P2 routes opt-in `xxt_compat` to the foreground app on port `6011`. The app uses a plain Vision request, compact BGRA `0x2002`, and automatic compute selection; background requests fail closed with `app_ocr_requires_foreground`. The 22:21 device canary succeeded with all five Core Video allocation probes returning zero and one recognized observation. The 22:49 qualification then found that a retained port `6011` can accept a connection without the app servicing it. Task `275` now performs a three-second live foreground bridge probe, and the app uses a lifecycle-backed atomic state instead of synchronously waiting on the main queue before its first breadcrumb. `Collect-TLinkVisionOCRQualification.ps1` provides a bounded 20 Fast + 1 Accurate + 1 large Fast foreground gate with task `97` postflight after every request; promotion remains blocked until that artifact and manual text/coordinate/background checks pass. Task `273/274` reads/clears the bounded debug log. See `docs/ocr-p1-cpu-only.md` and `docs/ocr-p2-xxt-compat.md`.
