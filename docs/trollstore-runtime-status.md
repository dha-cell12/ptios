...
- Keyboard/text: task `24` uses `clipboardd` v8 on port `6012` with private background Pasteboard entitlements. Every write is read-back verified before `streamd` trusts daemon reads. Subtask `1` inserts text using background clipboard plus HID `Command+V`; subtask `5` pastes the existing clipboard; subtasks `3/4` use HID arrows and Backspace. Devices that ignore the Pasteboard entitlements fall back to the foreground app bridge on port `6013`. Show/hide keyboard remains `limited_on_trollstore` because it needs the rootfull SpringBoard keyboard observer.
- Background UI bridge: `clipboardd` v8 receives toast/alert/dialog fallback events from `streamd`. UserNotifications rejects a UIDaemon audit identity with `UNErrorCodeNotificationsNotAllowed` even when the containing app is authorized, so v8 uses the existing `com.apple.springboard.CFUserNotification` entitlement to ask SpringBoard for a system notice/alert. Foreground still uses the UIKit overlay.
...
bringing StreamControl to the foreground. Task `249` should report `version=8`,
`background_visual_mode=cfusernotification_system_alert`, and finally
`background_visual_last=cfusernotification_result_0`. UserNotifications status
