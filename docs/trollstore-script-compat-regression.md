...
```powershell
# Open StreamControl.app once after install so privhelper installs clipboardd v6
...
Task `249` should contain `clipboard_backend_ready`, `version=6` in its decoded
...
- Keyboard API names are exposed for rootfull script compatibility. The v3
  ...
  not because background clipboard access is categorically impossible. Version 6
