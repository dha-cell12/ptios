...
The primary menu uses the proven `CFUserNotification` system-alert path so it
can be compositor-hosted while the app is backgrounded or locked; the secure
high-level UIKit window remains a fallback. This path does not load a
SpringBoard tweak, Substrate, or Activator. The
