...
Toast/alert/dialog/touch indicator 12, 22, 26, 42, 43: Xem chi tiết trong phần "Giao diện người dùng (UI Parity)". Foreground dùng overlay window của app-process. Background toast ưu tiên `TLinkUIService.app` với secure pass-through window; alert/dialog và lỗi UI service dùng CFUserNotification system alert.
...
- Foreground dependency reduction uses the clipboard UIDaemon pattern. `clipboardd` v15 keeps direct background Pasteboard access, handles best-effort keep-awake requests, routes background toast to a distinct hidden `TLinkUIService.app`, and adds a direct IOHID Volume Up double-click menu (`Launch / Record / Cancel`) without Substrate. The volume menu uses CFUserNotification as its compositor-hosted primary path and retains the secure UIWindow as fallback. StreamControl writes a short-lived foreground heartbeat so foreground and background feedback are not duplicated.
- A normal UIDaemon `UIWindow` was not reliably compositor-hosted. The new toast service separates UI ownership into a nested app running as mobile with `UIApplicationShowsViewsWhileLocked`, `SecureKey`, hidden app tag, platform/extension-host entitlements, a requested level `20000099.9` secure window (UIKit may report the clamped effective value `10000000`), and hit-test pass-through. It listens on port `6017`; `privhelper` starts it and `clipboardd` can respawn it. Foreground and background toast always use this route, avoiding the foreground-heartbeat race, and support `0` top, `1` center, `2` bottom; CFUserNotification remains the fixed-center failure fallback. Dialog button results are not yet bridged back to the original task.
...
- StreamControl registers `BGAppRefreshTask` and `BGProcessingTask` as a recovery fallback after first launch. The TrollStore package now also embeds `TLinkBootWidget.appex`: when the Home Screen widget is installed and Settings -> Boot Script is enabled, WidgetKit wakes the extension, the extension launches StreamControl through SpringBoardServices if task port 6000 is absent, and streamd runs the selected priority autolaunch script. This is widget-assisted/best-effort boot wake, not a platformized LaunchDaemon; diagnostics are stored in `/var/mobile/Library/TLinkauto/runtime/widget_boot_wake.plist` and `background_service_scheduler.plist`.
