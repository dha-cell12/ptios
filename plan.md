...
- A normal UIDaemon `UIWindow` was not reliably compositor-hosted. The new toast service separates UI ownership into a nested app running as mobile with `UIApplicationShowsViewsWhileLocked`, `SecureKey`, hidden app tag, platform/extension-host entitlements, a level `20000099.9` secure window, and hit-test pass-through. It listens on port `6017`; `privhelper` starts it and `clipboardd` can respawn it. Foreground and background toast support `0` top, `1` center, `2` bottom; CFUserNotification remains the fixed-center failure fallback. Dialog button results are not yet bridged back to the original task.
