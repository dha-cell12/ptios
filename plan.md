...
- Screenshot task 29 now supports action 1 file capture plus action 2 save-to-album and action 3 clear-album using the `TLinkauto` Photos album. If Photos access is blocked on a device/build, it must return a clear `save_album_failed` or `clear_album_failed` error.
