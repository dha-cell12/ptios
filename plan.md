...
- Foreground dependency reduction now uses the proven clipboard UIDaemon pattern. `clipboardd` v9 keeps direct background Pasteboard access and also handles background CFUserNotification system-alert fallback plus best-effort keep-awake requests. UserNotifications returned `UNErrorDomain` code 1 even with an explicit app bundle center and app authorization. `CFUserNotificationDisplayNotice` returned success without visible UI, so all background visual kinds now use `CFUserNotificationDisplayAlert`; toast uses no button and auto-dismisses. StreamControl writes a short-lived foreground heartbeat so foreground overlays and background system alerts are not duplicated.
