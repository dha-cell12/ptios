...
- Foreground dependency reduction uses the proven clipboard UIDaemon pattern. `clipboardd` v13 keeps direct background Pasteboard access, handles best-effort keep-awake requests, sends background toast/alert/dialog through CFUserNotification, and adds a direct IOHID Volume Up double-click menu (`Launch / Record / Cancel`) without Substrate. StreamControl writes a short-lived foreground heartbeat so foreground and background feedback are not duplicated.
