...
const output = resolve(args.output || `rootfull-license-phase1-${mode}.json`);
...
assert.equal(plistValue(buildXML, "RootfullLicensePhase"), 1);
assert.equal(plistValue(buildXML, "RootfullLicenseMode"), mode);
assert.equal(plistValue(buildXML, "LicenseContractVersion"), 1);
assert.equal(plistValue(buildXML, "EnforcementBehavior"), "shared_verifier_observe_no_runtime_gate");
...
const manifest = {
  ...
  license_mode: mode,
  integration_mode: "shared_verifier_observe_no_runtime_gate",
...
console.log(`rootfull phase 1 artifact OK: ${mode}, manifest ${output}`);
