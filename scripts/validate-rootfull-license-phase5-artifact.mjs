...
const output = resolve(args.output || `rootfull-license-phase5-${mode}.json`);
...
for (const relative of binaries) {
  ...
  for (const marker of evidence[relative] || []) {
    assert.ok(content.includes(marker), `${relative} lacks Phase 5 evidence ${marker}`);
  }
...
assert.equal(plistValue(buildXML, "RootfullLicensePhase"), 5);
...
const manifest = {
  ...
  ui_feature_snapshot_active: true,
  ui_main_thread_signature_verification: false,
  verifier_performance_metrics: {
    feature_check_average_and_max_us: true,
    cache_hit_and_miss: true,
    status_refresh_average_and_max_ms: true,
    ui_snapshot_refresh_average_and_max_ms: true,
  ...
  deferred_phase6: ["hardening_and_release_rollout"],
...
console.log(`rootfull phase 5 artifact OK: ${mode}, manifest ${output}`);
