...
assert.match(socket, /zx_rootfullPhase[23456]DiagnosticResponse/);
assert.match(socket, /rootfull_license_phase"\]\s*=\s*@[23456]/);
assert.match(socket, /activation_lifecycle_active"\]\s*=\s*@1/);
assert.match(socket, /license_phase=[23456]/);
...
assert.match(task, /licenseStatus\[@"phase"\]\s*=\s*@[23456]/);
...
assert.match(buildPlist, /<key>RootfullLicensePhase<\/key>\s*<integer>[23456]<\/integer>/);
...
assert.match(workflow, /validate-rootfull-license-phase[23456]-artifact\.mjs/);
