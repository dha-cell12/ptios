...
assert.match(socket, /zx_rootfullPhase[3456]DiagnosticResponse/);
assert.match(socket, /rootfull_license_phase"\]\s*=\s*@[3456]/);
...
const daemonDiagnosticIndex = socket.search(/NSData \*phase[456]Diagnostic/);
assert.ok(
  daemonGateIndex >= 0 &&
...
assert.match(task, /licenseStatus\[@"phase"\]\s*=\s*@[3456]/);
...
assert.match(buildPlist, /<key>RootfullLicensePhase<\/key>\s*<integer>[3456]<\/integer>/);
...
assert.match(workflow, /validate-rootfull-license-phase[3456]-artifact\.mjs/);
