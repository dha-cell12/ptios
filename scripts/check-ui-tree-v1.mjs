...
assert.match(trollServer, /AXSpringBoardServer/);
assert.match(trollServer, /AccessibilityUtilities\.framework/);
assert.match(trollServer, /focusedAppPID/);
assert.match(trollServer, /topEventPidOverride/);
assert.match(trollServer, /ax_springboard_focused_pid_v1/);
assert.match(trollServer, /TLinkUITreeContextByAXSpringBoardServer/);
assert.match(trollServer, /direct_foreground_state/);
assert.match(trollServer, /ui_ax_springboard_target_not_ready/);
assert.match(trollServer, /retryDelaysUs\[\][\s\S]*35 \* 1000[\s\S]*90 \* 1000/);
assert.match(trollServer, /authoritative_pid=1/);
assert.match(trollServer, /fingerprint_skipped=1/);
assert.match(trollServer, /axSpringBoardContext\[@"authoritative"\]/);
...
assert.match(trollServer, /com\.apple\.springboard/);
assert.match(trollServer, /excluded_services/);
assert.match(trollServer, /sTLinkUITreeCachedLaunchableBundleIds/);
assert.match(trollServer, /sTLinkUITreeCachedRunningContexts/);
assert.match(trollServer, /now - sTLinkUITreeLaunchableCacheAtMs <= 60000/);
assert.match(trollServer, /now - sTLinkUITreeRunningContextsCacheAtMs <= 750/);
assert.match(trollServer, /TLinkUITreePrioritizedCandidates/);
assert.match(trollServer, /sTLinkUITreeAcceptedBundleId/);
assert.match(trollServer, /ax_snapshot_stable_scan_v3/);
assert.match(trollServer, /ax_snapshot_mru_stable_v4/);
assert.match(trollServer, /ax_snapshot_early_stable_v4/);
assert.match(trollServer, /sbs_fbs_skipped_after_ax_retry/);
assert.match(trollServer, /task11_expected_stable_v3/);
assert.match(trollServer, /ui_frontmost_transition_unsettled/);
