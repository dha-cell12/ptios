...
assert.match(helper, /version=15/);
assert.match(streamd, /diagnostic containsString:@"version=15"/);
...
assert.match(workflow, /grep -q 'version=15' stream-app\/clipboardd\/main\.mm/);
assert.match(workflow, /registered_keyboard_page_12_usage_233/);
assert.ok(workflow.includes(`grep -q 'containsString:@"version=15"' stream-app/streamd/POCSocketServer.mm`));
assert.ok(workflow.includes(`grep -q 'containsString:@"version=15"' stream-app/privhelper/main.mm`));
