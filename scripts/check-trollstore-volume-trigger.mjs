...
assert.match(helper, /version=16/);
assert.match(streamd, /diagnostic containsString:@"version=16"/);
...
assert.match(workflow, /grep -q 'version=16' stream-app\/clipboardd\/main\.mm/);
assert.match(workflow, /registered_keyboard_page_12_usage_233/);
assert.ok(workflow.includes(`grep -q 'containsString:@"version=16"' stream-app/streamd/POCSocketServer.mm`));
assert.ok(workflow.includes(`grep -q 'containsString:@"version=16"' stream-app/privhelper/main.mm`));
