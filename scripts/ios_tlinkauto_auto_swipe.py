...
def run_swipe_points(client: TLinkautoSocket, args: argparse.Namespace, start_y: float, end_y: float) -> None:
...
def run_swipe(client: TLinkautoSocket, args: argparse.Namespace) -> None:
    run_swipe_points(client, args, args.start_y, args.end_y)


def run_bounce(client: TLinkautoSocket, args: argparse.Namespace) -> None:
    run_swipe_points(client, args, args.start_y, args.end_y)
    time.sleep(args.bounce_gap_ms / 1000.0)
    run_swipe_points(client, args, args.end_y, args.start_y)


def run_pan(client: TLinkautoSocket, args: argparse.Namespace) -> None:
    # Keep one continuous finger down while moving up/down several times. This creates
    # sustained screen motion and stresses TLinkauto without reconnecting the gesture.
    y = args.start_y
...
def run_tap(client: TLinkautoSocket, args: argparse.Namespace) -> None:
...
def run_one(client: TLinkautoSocket, args: argparse.Namespace) -> None:
    if args.mode == "swipe":
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Direct TLinkauto auto swipe/tap test over TCP 6000")
    ...
    client = TLinkautoSocket(args.host, args.port, args.protocol)
    client.connect()
    try:
