...
type LegacyApi = {
  openDeviceModal?: (serial: string) => void;
  disconnectDevice?: (serial: string) => void;
  openIosStream?: (device: UnifiedDevice) => void;
  refresh?: () => Promise<void> | void;
};

declare global {
  interface Window {
    __legacyDevices?: LegacyApi;
  }
}
...
export function DevicesPane() {
  ...
  const openModal = (serial: string) => window.__legacyDevices?.openDeviceModal?.(serial);
  const powerOff = (serial: string) => window.__legacyDevices?.disconnectDevice?.(serial);
  const openIos = (device: UnifiedDevice) => window.__legacyDevices?.openIosStream?.(device);
  const refresh = () => { void window.__legacyDevices?.refresh?.(); };
