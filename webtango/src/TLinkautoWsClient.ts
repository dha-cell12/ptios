...
export class TLinkautoWsClient {
  private url: string;
  ...
  async waitOpen(timeoutMs = 1500): Promise<void> {
    ...
    await Promise.race([
      new Promise<void>((resolve, reject) => {
        ...
        this.ws.addEventListener('error', () => reject(new Error('TLinkauto ws error')), { once: true });
      }),
      new Promise<void>((_, reject) => setTimeout(() => reject(new Error('TLinkauto ws open timeout')), timeoutMs)),
    ]);
  }
  ...
  private nextLine(timeoutMs = 1000): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(new Error('TLinkauto timeout'));
      }, timeoutMs);
