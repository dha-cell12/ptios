def format_socket_data(task_type, *datas):
    """Put data in correct format to send to TLinkauto tweak

    :param task_type: type of the task
    :param datas: data to be sent
    :return: TLinkauto socket data in correct format
    """
...
def decode_socket_data(data):
    ...
    if data[0] != "0":
        err_message = "Unknown err because TLinkauto doesn't send any error info to python"
        if len(temp) >= 2:
            err_message = temp[1]
