import time
import socket
from tlinkauto import tasktypes
from tlinkauto import datahandler
from tlinkauto import kbdtasktypes
from tlinkauto import deviceinfotasktypes
from tlinkauto import colorsearchtasktypes
from tlinkauto.screen import Screen
from tlinkauto.transport import TLinkautoTransport
from tlinkauto.app import App
import base64
import json
...
class TLinkauto:
    def __init__(self, ip):
        # Transport negotiates v1 JSON framing with v0 fallback.
        self.s = TLinkautoTransport.connect(str(ip), 6000)
        time.sleep(0.1)
    ...
    def turn_on_wifi(self):
        self.s.send(datahandler.format_socket_data(tasktypes.TASK_WIFI, 1, 1))
        return datahandler.decode_socket_data(self.s.recv(1024))

    def turn_off_wifi(self):
        self.s.send(datahandler.format_socket_data(tasktypes.TASK_WIFI, 1, 0))
        return datahandler.decode_socket_data(self.s.recv(1024))

    def is_wifi_on(self):
        self.s.send(datahandler.format_socket_data(tasktypes.TASK_WIFI, 0))
        result = datahandler.decode_socket_data(self.s.recv(1024))
        if not result[0]:
            return False, result[1]
        return True, (str(result[1][0]) == "1")

    def turn_on_bluetooth(self):
        self.s.send(datahandler.format_socket_data(tasktypes.TASK_BLUETOOTH, 1, 1))
        return datahandler.decode_socket_data(self.s.recv(1024))

    def turn_off_bluetooth(self):
        self.s.send(datahandler.format_socket_data(tasktypes.TASK_BLUETOOTH, 1, 0))
        return datahandler.decode_socket_data(self.s.recv(1024))

    def is_bluetooth_on(self):
        self.s.send(datahandler.format_socket_data(tasktypes.TASK_BLUETOOTH, 0))
        result = datahandler.decode_socket_data(self.s.recv(1024))
        if not result[0]:
            return False, result[1]
        return True, (str(result[1][0]) == "1")
    ...
    def turn_on_airplane(self):
        self.s.send(datahandler.format_socket_data(tasktypes.TASK_AIRPLANE, 1, 1))
        return datahandler.decode_socket_data(self.s.recv(1024))

    def turn_off_airplane(self):
        self.s.send(datahandler.format_socket_data(tasktypes.TASK_AIRPLANE, 1, 0))
        return datahandler.decode_socket_data(self.s.recv(1024))

    def is_airplane_on(self):
        self.s.send(datahandler.format_socket_data(tasktypes.TASK_AIRPLANE, 0))
        result = datahandler.decode_socket_data(self.s.recv(1024))
        if not result[0]:
            return False, result[1]
        return True, (str(result[1][0]) == "1")
    ...
    def turn_on_data(self):
        self.s.send(datahandler.format_socket_data(tasktypes.TASK_CELLULAR_DATA, 1, 1))
        return datahandler.decode_socket_data(self.s.recv(1024))

    def turn_off_data(self):
        self.s.send(datahandler.format_socket_data(tasktypes.TASK_CELLULAR_DATA, 1, 0))
        return datahandler.decode_socket_data(self.s.recv(1024))

    def is_data_on(self):
        self.s.send(datahandler.format_socket_data(tasktypes.TASK_CELLULAR_DATA, 0))
        result = datahandler.decode_socket_data(self.s.recv(1024))
        if not result[0]:
            return False, result[1]
        return True, (str(result[1][0]) == "1")
    ...
    def turn_on_vpn(self):
        self.s.send(datahandler.format_socket_data(tasktypes.TASK_VPN, 1, 1))
        return datahandler.decode_socket_data(self.s.recv(1024))

    def turn_off_vpn(self):
        self.s.send(datahandler.format_socket_data(tasktypes.TASK_VPN, 1, 0))
        return datahandler.decode_socket_data(self.s.recv(1024))

    def is_vpn_on(self):
        self.s.send(datahandler.format_socket_data(tasktypes.TASK_VPN, 0))
        result = datahandler.decode_socket_data(self.s.recv(1024))
        if not result[0]:
            return False, result[1]
        return True, (str(result[1][0]) == "1")
    ...
    def key_down(self, key_type):
        ...
        Args:
            key_type: one of HARDWARE_KEY_* values in tlinkauto.hardwarekeytypes
        """
        from tlinkauto import hardwarekeytypes
    ...
    def dialog(self, options):
        ...
        """
        title = options.get("title", "TLinkauto")
    ...
    def save_to_system_album(self, filePath):
        """Save an image to the TLinkauto album in Photos."""
    ...
    def clear_system_album(self):
        """Clear images saved to the TLinkauto album in Photos."""
    ...
    def hello_status(self):
        """Probe TLinkauto and get device/script status (base64 JSON)."""
        self.s.send(datahandler.format_socket_data(tasktypes.TASK_HELLO_STATUS))
        ok, res = datahandler.decode_socket_data(self.s.recv(1024 * 1024))
        if not ok:
            return False, res
        if not isinstance(res, list) or len(res) < 1:
            return False, "Invalid hello_status response"
        try:
            payload = base64.b64decode(str(res[0]).encode("ascii"), validate=False)
            return True, json.loads(payload.decode("utf-8", errors="strict"))
        except Exception as e:
            return False, "Failed to decode hello_status: " + str(e)
